...
def user_list():
    if request.method == 'GET':
        ...
        # 分页器对象
        paginate = UserModel.query.paginate(page=page, per_page=10)  # 当前页和每页展示多少数据
...
def user_search(user_id):
    if request.method == 'GET':
        userid = user_id
        username = user_id
        users = UserModel.query.filter(or_(UserModel.UserID == userid, UserModel.UserName == username))
